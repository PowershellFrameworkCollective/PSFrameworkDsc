Describe "Validating the module manifest" {
	$moduleRoot = (Resolve-Path "$global:testroot\..\PSFrameworkDsc").Path
	$manifest = ((Get-Content "$moduleRoot\PSFrameworkDsc.psd1") -join "`n") | Invoke-Expression
	Context "Basic resources validation" {
		...
		It "Exports all functions in the public folder" -TestCases @{ files = $files; manifest = $manifest } {
			if (-not $files) { $manifest.FunctionsToExport.Count | Should -BeLessThan 1 }
			else {
